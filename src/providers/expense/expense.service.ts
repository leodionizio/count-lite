...
export class ExpenseService {
		...
		if (this.isFirstCall) {
			...
			return this.sqliteConnService.getDb('finance.db').then((db: SQLiteObject) => {
				...
				this.db
					...
					.then(success => {
						console.log('Tabela criada gasto com sucesso', success);
					})
					.catch(error => {
						console.log('Erro ao criar tabela gasto', error);
					});
				return this.db;
			});
		}
		...
		return this.getDb()
			.then((db: SQLiteObject) => {
				return this.db
					.executeSql(
					`
						INSERT INTO expenses (month, description, value)
						VALUES (?, ?, ?)`,
	...
	public delete(id: number): Promise<boolean> {
		return this.getDb()
			.then((db: SQLiteObject) => {
				return this.db
					.executeSql('DELETE FROM expenses WHERE id=?', [id])
					.then(resultSet => resultSet.rowsAffected >= 0)
					.catch((error: Error) => {
